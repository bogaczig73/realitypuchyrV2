...
const { upload, deleteFile, getSignedUrl, uploadImageToS3 } = require('./services/s3Service');
...
app.post('/api/properties/:id/image', upload.single('image'), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const imageUrl = await uploadImageToS3(req.file, id);
